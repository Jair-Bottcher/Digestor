...
def consulta(
    ...
    if not EXCEL_PATH or not EXCEL_PATH.exists():
        raise HTTPException(status_code=404, detail="Arquivo de planilha não encontrado")
    try:
        df = pd.read_excel(EXCEL_PATH, sheet_name=EXCEL_SHEET, engine="openpyxl")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Erro ao ler planilha: {exc}")
    if limit <= 0:
    ...
    total_rows = len(df)
    start_idx = row_start if row_start is not None else 0
    end_idx = row_end if row_end is not None else total_rows

    if start_idx < 0 or start_idx > total_rows:
        raise HTTPException(status_code=400, detail="'row_start' fora dos limites")
    if end_idx < 0 or end_idx > total_rows:
        raise HTTPException(status_code=400, detail="'row_end' fora dos limites")
    if start_idx >= end_idx:
        raise HTTPException(status_code=400, detail="'row_start' deve ser menor que 'row_end'")

    df_slice = df.iloc[start_idx:end_idx]

    if columns:
        missing_cols = [col for col in columns if col not in df_slice.columns]
        if missing_cols:
            raise HTTPException(
                status_code=400,
                detail=f"Colunas não encontradas: {', '.join(missing_cols)}"
            )
        df_slice = df_slice.loc[:, columns]

    df_paginated = df_slice.iloc[offset: offset + limit]
    data = df_paginated.replace({np.nan: None}).to_dict(orient="records")
    return {
        "data": data,
        "metadata": {
            "total_rows": total_rows,
            "range_rows": len(df_slice),
